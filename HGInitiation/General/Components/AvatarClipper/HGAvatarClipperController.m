...
@interface HGAvatarClipperController ()

@property (strong, nonatomic) HGClipperView *photoView;

@end

@implementation HGAvatarClipperController
...
- (void)setupSubviews
{
    self.photoView = [[HGClipperView alloc] initWithFrame:self.view.bounds image:self.image maxRotationAngle:self.maxRotationAngle];
    self.photoView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    [self.view addSubview:self.photoView];
    
    
    UIButton *cancelBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    cancelBtn.frame = CGRectMake(8, CGRectGetHeight(self.view.frame) - 44 - 34, 60, 44);
    cancelBtn.titleLabel.textAlignment = NSTextAlignmentLeft;
    [cancelBtn setTitle:NSLocalizedStringFromTable(@"取消", @"cancel", nil) forState:UIControlStateNormal];
    ...
    if (@available(iOS 11.0, *)) {
        cropBtn.frame = CGRectMake(CGRectGetWidth(self.view.frame) - 60, CGRectGetHeight(self.view.frame) - 44 - 34, 60, 44);
    }else{
        cropBtn.frame = CGRectMake(CGRectGetWidth(self.view.frame) - 60, CGRectGetHeight(self.view.frame) - 44, 60, 44);
    }
    
    [cropBtn setTitle:NSLocalizedStringFromTable(@"完成", @"done", nil) forState:UIControlStateNormal];
...
{
    [self.delegate avatarClipperControllerDidCancel:self];
}
...
- (void)saveBtnTapped
{
    ...
    CGImageRef imageRef = [self newTransformedImage:transform
                                  ...
                                  sourceOrientation:self.image.imageOrientation
                                        outputWidth:fmin(self.image.size.width, self.image.size.height)
    ...
    [self.delegate avatarClipperController:self didFinishWithCroppedImage:image];
}
