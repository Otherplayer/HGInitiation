...
@implementation HGAvatarContentView
...
@property (nonatomic, strong) HGAvatarContentView *photoContentView;
...
{
    if ([self.delegate respondsToSelector:@selector(cropEnded:isLaunch:)]) {
        [self.delegate cropEnded:self isLaunch:NO];
    }
}
...
@interface HGClipperView () <UIScrollViewDelegate, CropViewDelegate>
             ...
             maxRotationAngle:(CGFloat)maxRotationAngle
{
    if (self = [super init]) {
        ...
        _photoContentView = [[HGAvatarContentView alloc] initWithImage:image];
...
//        [self addSubview:_resetBtn];
        ...
        dispatch_after(delayInNanoSeconds, concurrentQueue, ^(void){
            [self cropEnded:self.cropView isLaunch:YES];
        });
...
{
    CGFloat minWidth = fmin(self.cropView.bounds.size.width, self.cropView.bounds.size.height);
    
    self.cropView.frame = CGRectMake(self.cropView.frame.origin.x, self.cropView.frame.origin.y, minWidth, minWidth);
    [self updateMasks:NO];
}

- (void)cropEnded:(CropView *)cropView isLaunch:(BOOL)isLaunch
{
    
    CGFloat cropViewWidth = self.maximumCanvasSize.width;
    CGFloat cropViewHeight = self.maximumCanvasSize.width;
    
    // calculate the new bounds of crop view
    CGRect newCropBounds = CGRectMake(0, 0, cropViewWidth, cropViewHeight);
